def sum_arithmetic(n_terms, start_val, inc_val):
    ...
    return (n_terms-1)*inc_val+start_val+sum_arithmetic(n_terms-1,start_val,inc_val)
def sum_geometric(n_terms, start_val, mult_val):
    if n_terms==0: return 0
    return mult_val**(n_terms-1)*start_val+sum_geometric(n_terms-1,start_val,mult_val)
import sys
a = float(sys.argv[1])
b = float(sys.argv[2])
c = float(sys.argv[3])
